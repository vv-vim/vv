...
const measureCharSize = () => {
  ...
  bgEl.style.transform = `scale(${charWidth}, ${charHeight})`;
...
const redrawCmd = {
  ...
  scroll: ([scrollCount]) => {
    ...
    screenContext.drawImage(screenCanvasEl, x, y, w, h, X, Y, w, h);

    // Clear lines under scroll
    screenContext.fillStyle = defaultBgColor;
    screenContext.fillRect(cx, cy, cw, ch);
  },
  ...
  resize: (...props) => {
    ...
    screenCanvasEl.width = cols * charWidth;
    screenCanvasEl.height = rows * charHeight;

    bgEl.style.width = `${cols}px`;
    bgEl.style.height = `${rows}px`;

    bgCanvasEl.width = cols;
    bgCanvasEl.height = rows;

    bgContext.fillStyle = bgColor();
    bgContext.fillRect(0, 0, cols, rows);
  },
